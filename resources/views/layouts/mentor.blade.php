...
<head>
    ...
    <title>SPK-Fuzzy — @yield('title', 'Dashboard')</title>
...
<div class="flex min-h-screen">
    ...
    {{-- ── SIDEBAR ── --}}
    {{-- Mentor pakai --primary (#2B124C), bukan hitam gelap --}}
    <aside x-bind:class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
           class="w-[220px] min-h-screen bg-[#2B124C] fixed top-0 left-0 z-50 flex flex-col transition-transform duration-300">

        {{-- Logo --}}
        <div class="px-5 py-5 border-b border-white/[0.08] flex items-center justify-between">
            <div>
                <h1 class="text-[#FBE4D8] font-semibold text-sm tracking-[0.04em]">SPK-Fuzzy</h1>
        ...
        {{-- Role Badge — mentor tone --}}
        <div class="mx-4 mt-3 mb-1 px-2.5 py-1.5 rounded-lg flex items-center gap-2
                    bg-[#854F6C]/30 border border-[#854F6C]/50">
            <span class="w-[7px] h-[7px] rounded-full bg-[#DFB6B2] shrink-0"></span>
            <span class="text-[#DFB6B2] text-[10px] font-medium tracking-[0.06em] uppercase">Mentor</span>
        </div>
        ...
        <nav class="flex-1 px-3 py-3 space-y-0.5 overflow-y-auto">
            ...
            <a href="#"
               ...
               {{ request()->routeIs('mentor.dashboard') ? 'bg-[#854F6C]/55 text-[#FBE4D8]' : 'text-[#DFB6B2]/75 hover:bg-white/[0.06] hover:text-[#DFB6B2]' }}">
            ...
            <a href="#"
               class="flex items-center gap-2.5 px-3 py-2 rounded-[7px] text-[13px] transition-all duration-150
               {{ request()->routeIs('mentor.peserta.*') ? 'bg-[#854F6C]/55 text-[#FBE4D8]' : 'text-[#DFB6B2]/75 hover:bg-white/[0.06] hover:text-[#DFB6B2]' }}">
            ...
            <a href="#"
               class="flex items-center gap-2.5 px-3 py-2 rounded-[7px] text-[13px] transition-all duration-150
               {{ request()->routeIs('mentor.penilaian.*') ? 'bg-[#854F6C]/55 text-[#FBE4D8]' : 'text-[#DFB6B2]/75 hover:bg-white/[0.06] hover:text-[#DFB6B2]' }}">
            ...
            <a href="#"
               class="flex items-center gap-2.5 px-3 py-2 rounded-[7px] text-[13px] transition-all duration-150
               {{ request()->routeIs('mentor.hasil.*') ? 'bg-[#854F6C]/55 text-[#FBE4D8]' : 'text-[#DFB6B2]/75 hover:bg-white/[0.06] hover:text-[#DFB6B2]' }}">
                <i class="ti ti-certificate text-base shrink-0"></i>
                <span>Hasil &amp; Rekomendasi</span>
            </a>

            <p class="text-[10px] font-medium text-[#DFB6B2]/40 uppercase tracking-[0.08em] px-3 pt-4 pb-1.5">Akun</p>

            <a href="#"
               class="flex items-center gap-2.5 px-3 py-2 rounded-[7px] text-[13px] transition-all duration-150
               {{ request()->routeIs('mentor.profil') ? 'bg-[#854F6C]/55 text-[#FBE4D8]' : 'text-[#DFB6B2]/75 hover:bg-white/[0.06] hover:text-[#DFB6B2]' }}">
                <i class="ti ti-user text-base shrink-0"></i>
        ...
        <div class="px-3 py-3 border-t border-white/[0.08]">
            <div class="flex items-center gap-2.5 px-3 py-2 mb-1">
                <div class="w-8 h-8 rounded-full bg-[#854F6C] flex items-center justify-center text-[#FBE4D8] text-xs font-semibold shrink-0">
                    {{ strtoupper(substr(auth()->user()->name ?? 'M', 0, 2)) }}
                </div>
                <div class="overflow-hidden">
                    <p class="text-[#FBE4D8] text-[13px] font-medium truncate">{{ auth()->user()->name ?? 'Mentor' }}</p>
                    <p class="text-[#DFB6B2]/60 text-[11px]">Mentor Pembimbing</p>
                </div>
            </div>
